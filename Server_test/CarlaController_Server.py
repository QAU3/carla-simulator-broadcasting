...
class Car(object):
    def __init__(self,world,carType,carGen,carName,hud,gamma, conn):
        self.world=world
        try:
    ...
    def restart(self):
        ...
        self.rgb_observers=RGBObservers(self.player, self._conn)
...
class RGBObservers(object):
    def __init__(self, parent_actor, world_socket):
        self._world_socket=world_socket
        self.SENSOR_SEND_IP = "127.0.0.1"
    ...
    def set_sensors(self):
        ...
        lambda image: partial(self.rgb_callback,port=self._world_socket)(image))
        self.sensor.append(cam1)   
        
    def rgb_callback(self,image,port):

        self.send_image(image,port)
        # Set up the other data sensors for speed, battery etc
        ##Snehandjali code is demanding and is never closed properly  
...
class World(object):
    def __init__(self, carla_world, hud, args,conn):
        self.conn=conn
    ...
    def restart(self):
        ...
        car1=Car(self.world,self._actor_filter,self._actor_generation,"hero1",self.hud,self._gamma,self.conn)
